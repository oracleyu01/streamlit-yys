...
def bar_chart():
    ...
    fig = px.bar(df7, x='팀', y='승률' , color='승률', hover_data= ['승률','출루율'], labels={'승률':'한국 야구 승률 데이터'} , height=800)
...
with st.form(key ='Form1'):
    with st.sidebar:
        ...
        select_language =  st.sidebar.radio('데이터 분석 결과', ('금리와 집값 데이터 고객 맞춤 데이터 시각화', '한국 야구 데이터 고객 맞춤 데이터 시각화', '다른 데이터 분석'))
        
        
if select_language =='금리와 집값 데이터 고객 맞춤 데이터 시각화':  
    tab1, tab2 = st.tabs(["📈 Chart", "🗃 Data"])
   
    with tab1:
        tab1.subheader("금리가 집값과의 관계 고객 맞춤 데이터 시각화")
        plotting_demo()
    ...
    with tab1:
        tab1.subheader("한국 야구 데이터 고객 맞춤 데이터 시각화")
        bar_chart()
